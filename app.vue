  ...
  <div class="px-2 py-12 md:p-8 mx-auto md:max-w-[80%] overflow-hidden">
    <div class="border border-solid border-gray-300">
      ...
      <div class="grid md:grid-cols-5">
        <input class="border border-gray-300 p-3 m-2 outline-none" type="date" placeholder="Дата..." v-model="currentDate">
        <input class="border border-gray-300 p-3 m-2 outline-none" type="number" min="1" max="30" placeholder="№ к-ты..." v-model="currentRoom" />
        <input class="border border-gray-300 p-3 m-2 outline-none" type="number" min="1" max="3" placeholder="Этаж..." v-model="currentFloor" />
        <input class="border border-gray-300 p-3 m-2 outline-none" type="text" placeholder="Имя..." v-model="currentName" />
      ...
      <div class="flex md:flex-col overflow-x-scroll mt-5 md:mt-0 md:overflow-y-scroll">
        <div v-for="todo in filteredSchedule" :key="todo.id">
          <div class="border border-gray-300 border-t-0 grid md:grid-cols-5 min-w-[140px]">
            <span class="border-t md:border-t-0 md:border-l border-gray-300 p-5 md:first:border-none">{{ todo.date }}</span>
            <span class="border-t md:border-t-0 md:border-l border-gray-300 p-5 md:first:border-none">{{ todo.room }}</span>
            <span class="border-t md:border-t-0 md:border-l border-gray-300 p-5 md:first:border-none">{{ todo.floor }}</span>
            <span class="border-t md:border-t-0 md:border-l border-gray-300 p-5 md:first:border-none">{{ todo.name }}</span>
            ...
            <div v-if="showModal" class="w-full h-full fixed inset-0 bg-slate-600">
              <div class="grid bg-white w-full md:w-[40%] mx-auto top-[50%] translate-y-[-50%] relative p-5 pt-10">
                <input class="border border-gray-300 p-3 m-2 outline-none" type="date" placeholder="Дата..." v-model="modalDate">
                <input class="border border-gray-300 p-3 m-2 outline-none" type="number" min="1" max="30" placeholder="№ к-ты..." v-model="modalRoom" />
                <input class="border border-gray-300 p-3 m-2 outline-none" type="number" min="1" max="3" placeholder="Этаж..." v-model="modalFloor" />
                <input class="border border-gray-300 p-3 m-2 outline-none" type="text" placeholder="Имя..." v-model="modalName" />
